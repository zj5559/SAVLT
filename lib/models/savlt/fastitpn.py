...
class Fast_iTPN(nn.Module):
    ...
    def prepare_tokens_with_masks(self, template_list, search_list, template_anno_list, text_src,text_mask=None):
        ...
        z = z.view(-1, *z.size()[2:])  # (bn,c,h,w) (00,01,10,11,...b0,...bn)
        ...
        z = z.reshape(z.size(0), -1, z.size(-1))  # b,ln,c
        x = x.view(-1, num_search, x.size(-2), x.size(-1))
        x = x.reshape(x.size(0), -1, x.size(-1))

        if text_src is not None:
            if self.text_input_mode == 'concat':
                xz = torch.cat([x, z, text_src], dim=1)
            elif self.text_input_mode == 'add':
                xz = torch.cat([x, z], dim=1)
                xz += text_src
            elif self.text_input_mode == 'add_template':
                z += text_src
                xz = torch.cat([x, z], dim=1)
            elif self.text_input_mode == 'mul':
                xz = torch.cat([x, z], dim=1)
                xz = xz * text_src
            elif self.text_input_mode == 'mul_template':
                z = z * text_src
                xz = torch.cat([x, z], dim=1)
            else:
                raise NotImplementedError('text_input_mode should be choosen from concat, add, add_template, mul, mul_template')
        else:
            xz = torch.cat([x, z], dim=1)
        if text_mask is not None:
            img_mask=torch.zeros(z.shape[0], (x.shape[1]+z.shape[1]), dtype=int).to(x.device)
            attn_mask=torch.cat([img_mask,text_mask],dim=1)
        else:
            attn_mask=None

        if self.cls_token is not None:
            cls_tokens = self.cls_token.expand(B, -1, -1)
            if self.cls_pos=='start':
                xz = torch.cat([cls_tokens, xz], dim=1)
                if attn_mask is not None:
                    cls_mask=torch.zeros(z.shape[0], 1, dtype=int).to(x.device)
                    attn_mask=torch.cat([cls_mask,attn_mask],dim=1)
            elif self.cls_pos=='end':
                xz = torch.cat([xz,cls_tokens], dim=1)
                if attn_mask:
                    cls_mask=torch.zeros(z.shape[0], 1, dtype=int).to(x.device)
                    attn_mask=torch.cat([attn_mask,cls_mask],dim=1)

        return xz,attn_mask

    def forward_features(self, z,x, text_src,text_mask=None):
        B = z.shape[0]
        if text_src is not None:
            if self.text_input_mode == 'concat':
                xz = torch.cat([x, z, text_src], dim=1)
        ...
        for blk in self.blocks[-self.num_main_blocks:]:
            xz = checkpoint.checkpoint(blk, xz, rel_pos_bias) if self.grad_ckpt else blk(xz, rel_pos_bias,attn_mask=attn_mask)

        xz = self.norm(xz)

        if self.fc_norm is not None:
            return self.fc_norm(xz),xz_ori
        else:
            return xz,xz_ori

    def forward_features_early(self, template_list, search_list,template_anno_list):
        num_template = len(template_list)
        num_search = len(search_list)
    ...
    def forward(self, template_list=None, search_list=None, template_anno_list=None, mode='patch_embed',z=None,x=None,text_src=None,text_mask=None):
        if mode=='patch_embed':
            z,x = self.forward_features_early(template_list, search_list, template_anno_list)
            return z,x
        elif mode=='encoder':
            xz,xz_ori = self.forward_features(z,x, text_src,text_mask=text_mask)
            # x = self.head(x)
            out = [xz]
            return out,xz_ori
...
def fastitpnb(pretrained=False, pos_type="interpolate", pretrain_type="", patchembed_init="copy", **kwargs):
    ...
    if pretrained:
        import os
        current_dir = os.path.dirname(os.path.abspath(__file__))  # This is your Pro
        pretrained = os.path.join(current_dir, '../../../', pretrain_type)
