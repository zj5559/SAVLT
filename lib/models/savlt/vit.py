...
def vit_base_patch16(pretrained=False, pretrain_type='default',
                     search_size=384, template_size=192,
                     pos_type='interpolate', **kwargs):
    patch_size = 16
    ...
    if pretrained:
        load_pretrained(model, pretrain_type, pos_type,
...
def vit_huge_patch14(pretrained=False, pretrain_type='default',
                     search_size=364, template_size=182,
                     pos_type='interpolate', **kwargs):
    patch_size = 14
    model = VisionTransformer(
        search_size=search_size, template_size=template_size,
...
def load_pretrained(model, pretrain_type='default', pos_type='interpolate', cfg=None, num_classes=1000, in_chans=3, filter_fn=None, strict=True):
    if cfg is None:
