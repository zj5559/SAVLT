...
from lib.utils.box_ops import giou_loss,giou_loss_vec
...
from lib.utils.clip_loss import ClipLoss
from lib.train.actors import SAVLTActor
from lib.utils.focal_loss import FocalLoss,FocalLoss_vec
# for import modules
import importlib
...
def run(settings):
    ...
    if settings.script_name == "savlt":
        ...
        objective = {'giou': giou_loss, 'l1': l1_loss, 'focal': focal_loss, 'cls': BCEWithLogitsLoss(),'clip': clip_loss,'task_cls': CrossEntropyLoss(),}
        loss_weight = {'giou': cfg.TRAIN.GIOU_WEIGHT, 'l1': cfg.TRAIN.L1_WEIGHT, 'focal': 1.,
                       'cls': cfg.TRAIN.CE_WEIGHT,'task_cls': cfg.TRAIN.TASK_CE_WEIGHT,
                       'clip': cfg.TRAIN.CLIP_WEIGHT}
        actor = SAVLTActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings,
                                       cfg=cfg)
    else:
        raise ValueError("illegal script name")
