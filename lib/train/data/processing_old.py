        ...
        for s in s_list:
            ...
            if s=='search':
                ...
                for idx in range(len(data[s+'_anno'])):
                    if not data['search_visible'][idx]:
                        h,w,c=data['search_images'][idx].shape
                        jittered_anno.append(torch.Tensor([w/2,h/2,w/6,h/6]))
                        # print('here: rearrange the jittered anno')
                    else:
                        jittered_anno.append(self._get_jittered_box(data[s + '_anno'][idx], s))
