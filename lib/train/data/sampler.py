...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap,
                 ...
                 train_cls=False, pos_prob=0.5, multi_modal_language=False,allow_textlocal_rand=0.0,only_text_data=True,bert=False,grounding_ratio=0.0):
        """
        args:
        ...
        while not valid:
            try:
                    ...
                    if self.frame_sample_mode == 'causal':
                        ...
                        while search_frame_ids is None:
                            ...
                            if is_text_dataset and (visible.sum()<len(visible) and random.random()<self.allow_invisible):
                                ...
                                # if is_text_dataset and random.random() < self.allow_invisible:
                                #     invis = True
                ...
                if self.multi_modal_language:
                    ...
                    if cls_label==0:
                        nlp = ['The tracked object is currently invisible']
                    elif cls_label==2:
                        nlp = ['This is a tracking trap with similar distractors']
                    if nlp is not None:
                        nlp = nlp[0]

                    if self.bert:
                        nlp_ids, nlp_masks, num_nlp = self.extract_token_from_nlp_bert(nlp)
                    else:
                        nlp_ids, nlp_masks,num_nlp = self.extract_token_from_nlp_clip(nlp)
                    data['nlp_ids'] = nlp_ids
                    data['nlp_masks'] = nlp_masks
                    data['num_nlp'] = num_nlp
            ...
            try:
                ...
                if is_video_dataset:
                    ...
                    if self.frame_sample_mode == 'causal':
                        # Sample test and train frames in a causal manner, i.e. search_frame_ids > template_frame_ids
                        while search_frame_ids is None:
                            search_frame_ids = self._sample_visible_ids(visible, min_id=self.num_search_frames-1,
                                                                            max_id=len(visible),
                                                                            num_ids=self.num_search_frames)
                    elif self.frame_sample_mode == "order":
                        template_frame_ids, search_frame_ids = self.get_frame_ids_order(visible)
    ...
    def extract_token_from_nlp_bert(self, nlp,seq_length=40):
        if nlp is None:
            input_ids = torch.zeros(seq_length, dtype=torch.long)
            input_mask = torch.zeros(seq_length, dtype=torch.long)
            num_nlp=0
        else:
            # print('nlp',nlp)
            nlp = nlp.replace("_", " ")
        ...
        return input_ids, input_mask,num_nlp
