...
class STARKProcessing(BaseProcessing):
    ...
    def _get_jittered_box(self, box, mode):
        ...
        """
        if mode=='invis':
            scale_jitter_factor = 3.0
            center_jitter_factor =8.0
        else:
    ...
    def __call__(self, data: TensorDict):
        ...
        for s in ['template', 'search']:
            ...
            if s=='search' and data['cls_label']==0:
                #crop invisSearch
                jittered_anno = [self._get_jittered_box(a, 'invis') for a in data[s + '_anno']]
            else:
...
class SeqTrackProcessing(BaseProcessing):
    ...
    def __init__(self, search_area_factor, output_sz, center_jitter_factor, scale_jitter_factor,
                 mode='pair', multi_modal_language=False, settings=None, prob_temp=1.0,*args, **kwargs):
        """
        args:
            search_area_factor - The size of the search region  relative to the target size.
            output_sz - An integer, denoting the size to which the search region is resized. The search region is always
    ...
    def _get_jittered_box(self, box, mode):
        ...
        """
        if mode=='template':
            if random.random()<=self.prob_temp:
                scale_jitter_factor = 0.0
        ...
        for s in s_list:
            ...
            if s == 'search' and data['cls_label'] == 0:
                ...
                for idx in range(len(data[s + '_anno'])):
                    hh, ww, c = data['search_images'][idx].shape
                    jittered_anno.append(self._get_jittered_box(torch.Tensor(
                        [random.randint(0, int(ww - ww // 10)), random.randint(0, int(hh - hh // 10)), ww // 10,
                         hh // 10]), s))
            else:
                jittered_anno = [self._get_jittered_box(a, s) for a in data[s + '_anno']]

            if s=='template':
                data[s + '_anno']=jittered_anno
            # 2021.1.9 Check whether data is valid. Avoid too small bounding boxes
            w, h = torch.stack(jittered_anno, dim=0)[:, 2], torch.stack(jittered_anno, dim=0)[:, 3]
            ...
            crops, boxes, att_mask, mask_crops = prutils.jittered_center_crop(data[s + '_images'], jittered_anno,
                                                                              ...
                                                                              self.output_sz[s], masks=data[s + '_masks'])
            # if s=='search' and data['cls_label']==0:
            #     x1=min(max(0,boxes[0][0]),1)
            #     y1 = min(max(0, boxes[0][1]),1)
            #     x2=max(0,min(boxes[0][0]+boxes[0][2],1))
            #     y2 = max(0,min(boxes[0][1] + boxes[0][3], 1))
            #     iou,_=box_iou(torch.tensor([[x1,y1,x2,y2]]), torch.tensor([[boxes[0][0],boxes[0][1],boxes[0][0]+boxes[0][2],boxes[0][1] + boxes[0][3]]]))
            #     if iou>0.3:
            #         print('--change invis to vis:',iou)
            #         data['cls_label'] = 1
            #         # print('here')
            #         vis=False
            #         if vis and s=='search':
            #             import cv2
            #             import os
            #             path='/media/zj/ssd/models/vis_lang'
            #             img=cv2.rectangle(crops[0],(int(boxes[0][0]*224),int(boxes[0][1]*224)),(int((boxes[0][0]+boxes[0][2])*224),int((boxes[0][1]+boxes[0][3])*224)),(0,255,0),1)
            #             a=len(os.listdir(path))+1
            #             cv2.imwrite(os.path.join(path,str(a)+'.jpg'),img)
    ...
    def grounding_process(self, data: TensorDict):
        ...
        grounding_resize = [prutils.grounding_resize(im, self.output_sz['search'], box)
                            for im, box in zip(data['search_images'], data['search_anno'])]
        resize_grounding_frames, resize_grounding_box, _,grounding_mask, _ = zip(*grounding_resize)

        #vis
        # import cv2
        # im=cv2.rectangle(resize_grounding_frames[0],(int(resize_grounding_box[0][0]*224),int(resize_grounding_box[0][1]*224)),
        #                  (int((resize_grounding_box[0][0]+resize_grounding_box[0][2])*224),int((resize_grounding_box[0][1]+resize_grounding_box[0][3])*224)),
        #                  (0,255,0),2)
        # cv2.imwrite('/media/zj/T9/vis/test.jpg',im)
        # data['search_images']=resize_grounding_frames
        # data['search_anno']=resize_grounding_box
        # data['search_masks']=grounding_att_mask
        data['search_images'], data['search_anno'], data['search_masks'] = \
            self.transform['grounding'](image=resize_grounding_frames, bbox=resize_grounding_box,
                                        mask=grounding_mask, joint=False)
