...
class Got10k(BaseVideoDataset):
    ...
    def __init__(self, root=None, image_loader=jpeg4py_loader, split=None, seq_ids=None, data_fraction=None,
        ...
        self.dense_nlp_root = '/media/zj/T9/Dataset/tracking/lang-data/lang-anno/ours/got10k'
    ...
    def _read_nlp_dense(self, nlp_file):
        try:
            nlp = pandas.read_csv(nlp_file, dtype=str, header=None, low_memory=False,sep='|').values
        except:
            print(nlp_file)
        out_dict={}
        for text in nlp:
    ...
    def get_sequence_info(self, seq_id):
        ...
        output={'bbox': bbox, 'valid': valid, 'visible': visible, 'visible_ratio': visible_ratio}
        if self.multi_modal_language and self.use_nlp:
            nlp_path=os.path.join(self.dense_nlp_root, self.sequence_list[seq_id]+'.txt')
            nlp = self._read_nlp_dense(nlp_path)
            output['nlp'] = nlp[0] #official_nlp
            output['nlp_dense']=nlp #dense_nlp
    ...
    def get_frames(self, seq_id, frame_ids, anno=None):
        ...
        for key, value in anno.items():
            if key == 'nlp':
                ...
                anno_frames['nlp_local'] = [value[max(0,(int(f_id/100)-1))*100] for f_id in frame_ids]
                anno_frames['nlp_curr'] = []
                for f_id in frame_ids:
                    if not anno['visible'][f_id]:
                        anno_frames['nlp_curr'].append('the object is invisible.')
                    else:
                        anno_frames['nlp_curr'].append(value[max(0, (int(f_id / 100))) * 100])
            else:
