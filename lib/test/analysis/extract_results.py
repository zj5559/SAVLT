...
def extract_results(trackers, dataset, report_name, skip_missing_seq=False, plot_bin_gap=0.05,
    ...
    for seq_id, seq in enumerate(tqdm(dataset)):
        ...
        for trk_id, trk in enumerate(trackers):
            # Load results
            if seq.dataset in ['trackingnet', 'lasot', 'got10k', 'lasot_extension_subset', 'otb', 'uav',
                               'nfs', 'tnl2k','lasotextlang','lasotlang','otb99lang'] and '_onlynl' not in report_name:
                base_results_path = os.path.join(trk.results_dir, seq.dataset, seq.name)
            elif '_onlynl' in report_name:
                base_results_path = os.path.join(trk.results_dir, seq.dataset+'_onlynl', seq.name)
            else:
