...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict):
    ...
    if seq.dataset in ['trackingnet', 'got10k', 'lasot', 'lasot_extension_subset', 'otb', 'uav', 'nfs', 'tnl2k','lasotextlang','lasotlang','otb99lang','tnl2k_onlynl','lasotextlang_onlynl','lasotlang_onlynl','otb99lang_onlynl']:
    ...
    for key, data in output.items():
        ...
        #
        #         for obj_id, d in data_dict.items():
        #             bbox_file = '{}_{}_vis.txt'.format(base_results_path, obj_id)
        #             save_score(bbox_file, d)
        #     else:
        #         # Single-object mode
        #         print("saving pred_visible...")
        #         bbox_file = '{}_vis.txt'.format(base_results_path)
        #         save_score(bbox_file, data)
        #
        # if key == 'best_score':
        #     if isinstance(data[0], (dict, OrderedDict)):
        ...
        #
        #         for obj_id, d in data_dict.items():
        #             bbox_file = '{}_{}_score.txt'.format(base_results_path, obj_id)
        #             save_score(bbox_file, d)
        #     else:
        #         # Single-object mode
        #         print("saving scores...")
        #         bbox_file = '{}_score.txt'.format(base_results_path)
        #         save_score(bbox_file, data)
...
def run_sequence(seq: Sequence, tracker: Tracker, debug=False, num_gpu=8):
    ...
    def _results_exist():
        if seq.object_ids is None:
            if seq.dataset in ['trackingnet', 'got10k', 'lasot', 'lasot_extension_subset', 'otb', 'uav',
                               'nfs', 'tnl2k','lasotextlang','lasotlang','otb99lang', 'tnl2k_onlynl','lasotextlang_onlynl','lasotlang_onlynl','otb99lang_onlynl']:
                base_results_path = os.path.join(tracker.results_dir, seq.dataset, seq.name)
                bbox_file = '{}.txt'.format(base_results_path)
            else:
